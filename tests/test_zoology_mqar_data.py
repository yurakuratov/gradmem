...
def test_context_noise_preserves_signal_and_query_labels():
    ...
    assert clean.inputs.shape == (4, 20)
    assert noisy.inputs.shape == (4, 24)
    assert clean.slices["context_size"] == 16
    assert noisy.slices["clean_context_size"] == 16
    assert noisy.slices["context_size"] == 20
    assert noisy.slices["noise_tokens"] == 4
    assert len(noisy.slices["noise_gap_counts"]) == 5

    noisy_context = noisy.inputs[:, :20]
    noise_tokens = []
    offset = 0
    for pair_index, gap_count in enumerate(noisy.slices["noise_gap_counts"][:-1]):
        ...
        assert torch.all(pair[:, 0] == PAIR_OPEN_TOKEN)
        assert torch.all(pair[:, 3] == PAIR_CLOSE_TOKEN)
        assert torch.equal(pair, clean.inputs[:, pair_index * 4:pair_index * 4 + 4])
        offset += 4
    noise_tokens.append(noisy_context[:, offset:])
    noise_tokens = torch.cat(noise_tokens, dim=1)
    assert noise_tokens.shape[1] == 4
    assert torch.all((noise_tokens != PAIR_OPEN_TOKEN) & (noise_tokens != PAIR_CLOSE_TOKEN))
    assert torch.any(noise_tokens < 32)
    assert torch.any(noise_tokens >= 32)
    assert torch.equal(noisy.inputs[:, 20:], clean.inputs[:, 16:])
    assert torch.equal(noisy.labels[:, 20:], clean.labels[:, 16:])
    assert torch.all(noisy.labels[:, :20] == -100)
...
def test_context_noise_is_deterministic_and_zero_is_unchanged():
    ...
    noisy_a = _make_dense(noise_lvl=1.0)
    noisy_b = _make_dense(noise_lvl=1.0)
...
@pytest.mark.parametrize("noise_lvl", [-0.01, 1.01, float("nan")])
