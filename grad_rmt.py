...
class GradRMT(PreTrainedModel):
    ...
    def _adam_step(self, p, g, state, step_idx, lr, beta1=0.9, beta2=0.999, eps=1e-8, clip_value=10.0):
        ...
        # ---- 3. write back buffers ---------------------------- #
        if self.grad_mode == "first":
    ...
    def _sgd_step(self, p, g, state, lr, beta=0.9, nesterov=True, clip_value=None, clip_norm=None):
        """
        ...
        if self.momentum_mode != "none":
            m.lerp_(g, 1 - beta)
            update = g.lerp_(m, beta) if nesterov else m
        else:
            update = g

        if self.grad_mode == "first":
            state["m"].copy_(m.detach())
        elif self.grad_mode == "second":
            state["m"].copy_(m)
    ...
    def _muon_step(self, p, g, state, lr, beta=0.95, nesterov=True, ns_steps=5, eps=1e-8, clip_value=10.0):
        if "m" not in state:
            state["m"] = torch.zeros_like(p, memory_format=torch.preserve_format)
        m = state["m"]
    ...
    def forward(self, input_ids, labels=None, return_mem=False):
        ...
        context_segments = self.split_tensor(context_input_ids)
        ...
        for i, segment_input_ids in enumerate(context_segments):
            if self.K and segment_input_ids.ne(pad_id).any():
                inner_loop_kwargs = {
                    'context_input_ids': segment_input_ids,
                    'mem_batch': mem_batch,
                    'opt_state': opt_state,
                }
                if self.mem_proj_mode == 'per_sample':
    ...
    def split_tensor(self, tensor):
        split_inds = list(range(0, tensor.shape[1], self.segment_size)) + [tensor.shape[1]]
