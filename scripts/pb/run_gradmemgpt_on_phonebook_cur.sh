...
N_PAIRSS=(0 2 4 8 16 32 64)

for (( j=1; j<${#N_PAIRSS[@]}; j++ )); do

N_PAIRS=${N_PAIRSS[j]}
PREV_N_PAIRS=${N_PAIRSS[j-1]}
...
if [ "$j" -gt 1 ]; then
  PREV_PREV_N_PAIRS=${N_PAIRSS[j-2]}
  PREV_RUN_NAME="${RUN_NAME}_init_${PREV_PREV_N_PAIRS}p"
else
  PREV_RUN_NAME=None
fi
RUN_NAME="${RUN_NAME}_init_${PREV_N_PAIRS}p"

DATA_NAME="phonebook_N${N_PAIRS}"
PREV_DATA_NAME="phonebook_N${PREV_N_PAIRS}"
...
for N in "${N_VALUES[@]}"; do
  ...
  CPT_PATH="/cephfs/home/mkairov/gd_runs/${PREV_DATA_NAME}/${PREV_RUN_NAME}/run_$N"
  ...
  accelerate launch \
    ...
    --seed $((142+$N)) \
    $( [ $PREV_N_PAIRS != 0 ] && echo "--init_checkpoint $CPT_PATH" )
