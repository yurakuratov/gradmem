...
# Shared implementation for the three AR-multihop curriculum entry points.
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
  echo "Source this helper from an AR-multihop curriculum entry point." >&2
  exit 2
fi
...
DATASET_FORMAT=${DATASET_FORMAT:-ar_multihop}
DATASET_VOCAB_SUFFIX=${DATASET_VOCAB_SUFFIX:-16}
MODEL_VOCAB_SIZE=${MODEL_VOCAB_SIZE:-27}
DEFAULT_HF_DATASET=${DEFAULT_HF_DATASET:-irodkin/ar_multihop}
DEFAULT_OUTPUT_ROOT=${DEFAULT_OUTPUT_ROOT:-$REPO_ROOT/runs/energy_gradmem_ar_multihop_curriculum}
DEFAULT_WANDB_PROJECT=${DEFAULT_WANDB_PROJECT:-ar_multihop}
RUN_NAME_PREFIX=${RUN_NAME_PREFIX:-energy_gradmem_ar_multihop}
INCLUDE_HOP_IN_WANDB_NAME=${INCLUDE_HOP_IN_WANDB_NAME:-true}
CURRICULUM_GATE_SCRIPT=${CURRICULUM_GATE_SCRIPT:-$REPO_ROOT/ar_multihop_curriculum_gate.py}
...
# Small AR-multihop-compatible decoder.
L=${L:-2}
N_HEAD=${N_HEAD:-1}
D=${D:-128}
...
if ! [[ "$HOP_LENGTH" =~ ^[0-9]+$ ]] || [ "$HOP_LENGTH" -lt 1 ]; then
...
for RUN_ID in $RUN_NUMBERS; do
  ...
  for STAGE_INDEX in "${!CURRICULUM_N[@]}"; do
    ...
    if [ $((DATASET_N % HOP_LENGTH)) -ne 0 ]; then
      echo "HOP_LENGTH=$HOP_LENGTH must divide stage N=$DATASET_N" >&2
      exit 1
    fi
    ...
    if [ "$STAGE" -lt "$START_STAGE" ]; then
      ...
      echo "Starting seg=$SEGMENT_REGIME run=$RUN_ID stage=$STAGE N=$DATASET_N H=$HOP_LENGTH segments=$SEGMENT_COUNT CE=$CE_WEIGHT inner_lr=$INNER_LR"
      WANDB_NAME="$WANDB_NAME" "$PYTHON_BIN" -m accelerate.commands.launch \
        --main_process_port "$PORT" \
        --num_processes "$NP" \
        --mixed_precision "$MIXED_PRECISION" \
        --config_file "$REPO_ROOT/accelerate.yaml" \
        "$REPO_ROOT/run_energy_gradmem_on_ar_multihop.py" \
    ...
    if [ "$STAGE_WAS_TRAINED" = "true" ]; then
      if "$PYTHON_BIN" "$CURRICULUM_GATE_SCRIPT" \
