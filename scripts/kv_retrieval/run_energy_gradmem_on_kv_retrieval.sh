...
N_PAIRS_IN_SEGMENT=${N_PAIRS_IN_SEGMENT:-8}
N_SEGMENTS_IN_CONTEXT=${N_SEGMENTS_IN_CONTEXT:-1}

N_PAIRS=${N_PAIRS:-$((N_PAIRS_IN_SEGMENT*N_SEGMENTS_IN_CONTEXT))}
K_SIZE=${K_SIZE:-2}
V_SIZE=${V_SIZE:-2}
...
ADD_INNER_LOSS_TO_OUTER=${ADD_INNER_LOSS_TO_OUTER:-false}
...
# Energy objective.
INNER_OBJECTIVE=${INNER_OBJECTIVE:-neural}
ENERGY_HIDDEN_SIZE=${ENERGY_HIDDEN_SIZE:-$D}
ENERGY_NUM_LAYERS=${ENERGY_NUM_LAYERS:-2}
ENERGY_DROPOUT=${ENERGY_DROPOUT:-0.0}
if [ "$SEGMENT_WRITE_MODE" = "parallel" ]; then
  ENERGY_FUTURE_MODE=${ENERGY_FUTURE_MODE:-none}
else
  ENERGY_FUTURE_MODE=${ENERGY_FUTURE_MODE:-next_token}
fi
ENERGY_CE_GUIDANCE=${ENERGY_CE_GUIDANCE:-false}
ENERGY_CE_GUIDANCE_ALPHA=${ENERGY_CE_GUIDANCE_ALPHA:-0.0}
ENERGY_INNER_CE_WEIGHT=${ENERGY_INNER_CE_WEIGHT:-0.0}
ENERGY_WEIGHT_RMS_REG=${ENERGY_WEIGHT_RMS_REG:-0.0}
ENERGY_WEIGHT_RMS_THRESHOLD=${ENERGY_WEIGHT_RMS_THRESHOLD:-0.0}
ENERGY_DELTA_REG=${ENERGY_DELTA_REG:-0.0}
ENERGY_DELTA_MAX=${ENERGY_DELTA_MAX:-1.0}
ENERGY_REPLAY_WEIGHT=${ENERGY_REPLAY_WEIGHT:-0.0}
ENERGY_MODEL_TYPE=${ENERGY_MODEL_TYPE:-lstm}
ENERGY_SEGMENT_STATE_SIZE=${ENERGY_SEGMENT_STATE_SIZE:-$D}
ENERGY_MAMBA_STATE_SIZE=${ENERGY_MAMBA_STATE_SIZE:-128}
ENERGY_MAMBA_CONV_KERNEL=${ENERGY_MAMBA_CONV_KERNEL:-4}
ENERGY_MAMBA_EXPAND=${ENERGY_MAMBA_EXPAND:-2}
ENERGY_MAMBA_HEAD_DIM=${ENERGY_MAMBA_HEAD_DIM:-64}
ENERGY_MAMBA_CHUNK_SIZE=${ENERGY_MAMBA_CHUNK_SIZE:-256}
ENERGY_MAMBA_BACKEND=${ENERGY_MAMBA_BACKEND:-cuda}
if [ "$ENERGY_MODEL_TYPE" = "segment_delta_gru" ]; then
  ENERGY_PRETRAIN_OBJECTIVE=${ENERGY_PRETRAIN_OBJECTIVE:-ce}
  ENERGY_PRETRAIN_STEPS=${ENERGY_PRETRAIN_STEPS:-0}
elif [ "$SEGMENT_WRITE_MODE" = "parallel" ]; then
  ENERGY_PRETRAIN_OBJECTIVE=${ENERGY_PRETRAIN_OBJECTIVE:-ce}
  ENERGY_PRETRAIN_STEPS=${ENERGY_PRETRAIN_STEPS:-0}
elif [ "$INNER_OBJECTIVE" = "neural" ]; then
  ENERGY_PRETRAIN_OBJECTIVE=${ENERGY_PRETRAIN_OBJECTIVE:-embedding_l2}
  ENERGY_PRETRAIN_STEPS=${ENERGY_PRETRAIN_STEPS:-10000}
else
  ENERGY_PRETRAIN_OBJECTIVE=${ENERGY_PRETRAIN_OBJECTIVE:-ce}
  ENERGY_PRETRAIN_STEPS=${ENERGY_PRETRAIN_STEPS:-0}
fi
ENERGY_PRETRAIN_BATCH_SIZE=${ENERGY_PRETRAIN_BATCH_SIZE:-64}
ENERGY_PRETRAIN_SEQ_LEN=${ENERGY_PRETRAIN_SEQ_LEN:-32}
ENERGY_PRETRAIN_LR=${ENERGY_PRETRAIN_LR:-1e-3}
ENERGY_PRETRAIN_SEED=${ENERGY_PRETRAIN_SEED:-0}
if [ "$ENERGY_MODEL_TYPE" = "segment_delta_gru" ]; then
  ENERGY_PRETRAIN_L2_REG=${ENERGY_PRETRAIN_L2_REG:-0.0}
elif [ "$INNER_OBJECTIVE" = "neural" ]; then
  ENERGY_PRETRAIN_L2_REG=${ENERGY_PRETRAIN_L2_REG:-0.1}
else
  ENERGY_PRETRAIN_L2_REG=${ENERGY_PRETRAIN_L2_REG:-0.0}
fi
if [ "$ENERGY_MODEL_TYPE" = "segment_delta_gru" ]; then
  ENERGY_FREEZED_STEPS=${ENERGY_FREEZED_STEPS:-0}
elif [ "$INNER_OBJECTIVE" = "neural" ]; then
  ENERGY_FREEZED_STEPS=${ENERGY_FREEZED_STEPS:-10000}
else
  ENERGY_FREEZED_STEPS=${ENERGY_FREEZED_STEPS:-0}
fi
...
export WANDB_NAME=${WANDB_NAME:-${MODEL}_N${N_PAIRS_IN_SEGMENT}x${N_SEGMENTS_IN_CONTEXT}${ENERGY_ARCH_SUFFIX}}
...
for N in $N_VALUES; do
  ...
  $PYTHON_BIN -m accelerate.commands.launch \
    ...
    --energy_ce_guidance "$ENERGY_CE_GUIDANCE" \
    --energy_ce_guidance_alpha "$ENERGY_CE_GUIDANCE_ALPHA" \
    --energy_inner_ce_weight "$ENERGY_INNER_CE_WEIGHT" \
    ...
    --energy_mamba_state_size "$ENERGY_MAMBA_STATE_SIZE" \
    --energy_mamba_conv_kernel "$ENERGY_MAMBA_CONV_KERNEL" \
    --energy_mamba_expand "$ENERGY_MAMBA_EXPAND" \
    --energy_mamba_head_dim "$ENERGY_MAMBA_HEAD_DIM" \
    --energy_mamba_chunk_size "$ENERGY_MAMBA_CHUNK_SIZE" \
    --energy_mamba_backend "$ENERGY_MAMBA_BACKEND" \
    --energy_pretrain_objective "$ENERGY_PRETRAIN_OBJECTIVE" \
    --energy_pretrain_steps "$ENERGY_PRETRAIN_STEPS" \
    --energy_pretrain_batch_size "$ENERGY_PRETRAIN_BATCH_SIZE" \
    --energy_pretrain_seq_len "$ENERGY_PRETRAIN_SEQ_LEN" \
    --energy_pretrain_lr "$ENERGY_PRETRAIN_LR" \
    --energy_pretrain_seed "$ENERGY_PRETRAIN_SEED" \
    --energy_pretrain_l2_reg "$ENERGY_PRETRAIN_L2_REG" \
    --energy_freezed_steps "$ENERGY_FREEZED_STEPS" \
    --max_steps "$MAX_STEPS" \
