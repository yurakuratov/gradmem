...
# Add LaCT to the Python path
sys.path.insert(0, str(Path(__file__).parent / "LaCT" / "lact_llm"))
from lact_model import LaCTSWIGLUConfig, LaCTForCausalLM
...
class ExperimentArgs:
    ...
    # Architecture parameters (mapped from ARMT defaults)
    n_layer: Optional[int] = field(default=4)
    n_embd: Optional[int] = field(default=128)
    # LaCT-specific parameters
    num_attn_heads: Optional[int] = field(default=4)
    num_lact_heads: Optional[int] = field(default=4)
    inter_multi: Optional[int] = field(default=1)
    lact_chunk_size: Optional[int] = field(default=16)
    window_size: Optional[int] = field(default=64)
    use_muon: Optional[bool] = field(default=True)
    use_momentum: Optional[bool] = field(default=True)
...
if __name__ == '__main__':
    ...
    def build_lact_config():
        return LaCTSWIGLUConfig(
            ...
            # Disable fla fused kernels for simplicity
    ...
    if args.model_cpt is not None and args.model_cpt != 'None':
        ...
        logger.info(f'Loading LaCT model from checkpoint: {checkpoint_dir}')
        try:
            model = LaCTForCausalLM.from_pretrained(checkpoint_dir)
            logger.info(f'Successfully loaded LaCT model from {checkpoint_dir}')
        except Exception as e:
            logger.warning(f'Failed to load as pretrained model: {e}')
            logger.info('Trying to load from state dict files...')
            lact_config = build_lact_config()
            model = LaCTForCausalLM(lact_config)
            checkpoint_paths = [
