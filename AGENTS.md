...
7. **Pair-aware segmentation** (`grad_memgpt.py`, kv-retrieval; default-off). `gradmem.pair_aware_segmentation: true` chunks the WRITE context at `!K:V!` pair boundaries instead of fixed-width token windows. Motivation (measured on N8-K2V2, 4×16-token windows): the window boundaries at tokens 16/32/48 cut pair p4 between its key (in segment 1) and its value (in segment 2) — no Hopfield slot then contains both the key and the answer, so every `?!AK:`-style query for p4 is structurally unretrievable → the forgetting-matrix row 2 is depressed by ~25 points in EVERY seed (deterministic data geometry, not training variance; verified: `python` diagnostic regexing pair spans vs window spans). `segment_size=14` fixes N8 by phase luck (S=64 ≡ 1 mod 7) but breaks at other N (curriculum); pair-aware chunking is the robust fix.
   - **Config:** `pair_aware_segmentation` + exactly one sizing mode: `pairs_per_segment: N` (n_segments = ceil(n_pairs/N) per sample) or neither → divide each sample's pairs equally over `hopfield_n_segments` (first n%g groups get the extra pair: 8 pairs / 4 → 2/2/2/2, 8/3 → 3/3/2). `pair_delim_token_ids` (['!'-id, ':'-id]) is auto-filled by the runner from the tokenizer; mutually exclusive with `hopfield_segment_size`.
   - **Mechanics:** `_detect_pair_ends` scans context_input_ids with a state machine mirroring the collator regex `!([^!|:]+):([^!|]+)!` (no tokenizer in the model, no new batch plumbing — eval/forgetting paths get the chunking for free); the last pair's end extends to the sample's real end so the trailing `|` joins the final segment. `_divide_pairs` (staticmethod, shared VERBATIM by `build_kv_probe_queries` — the collator attributes seg_idx by pair INDEX through the same division, so alignment holds by construction). `_pair_segment_spans` builds per-sample (start,end) token spans; per-segment tensors are right-padded within the window to the batch-max group width (real tokens at the left → cumsum position ids give 0..k-1 exactly). Batches with different pair counts per sample are fine (per-sample division; empty tail groups → the existing all-pad skip + None snapshots).
   - Also fixed en route: `_compress_query` now clamps position ids at 0 — an INACTIVE all-pad probe slot (exists whenever a batch mixes pair counts) previously produced position_id=-1 and crashed the position-embedding gather; no-op for real READ queries.
   - Tests: `test_grad_memgpt_pair_aware.py` (detection, division, spans, collator==model no-straddle invariant, forward purity + forgetting triples under the mode, variable pair counts, config guards, adaptive-fork port).
8. **Unified segmentation surface (both models).** ONE top-level `segmentation:` YAML section — `n_segments`, `segment_size`, `pair_aware_segmentation`, `pairs_per_segment` — controls WRITE-context chunking in BOTH `grad_memgpt.py` and `grad_memgpt_adaptive.py` (the fork gained the pair-aware mode; its carried memory now consumes whole pairs per gated update). `run_from_config.py` flattens the section into the unified CLI args; the runner maps them onto the main model's `hopfield_n_segments`/`hopfield_segment_size` config knobs and passes them natively to the fork. `--hopfield-n-segments`/`--hopfield-segment-size` are deprecated aliases (adopted at startup with a log line; mixing old+new raises). kv_retrieval configs migrated (`adaptive_star/stage0/curriculum/mamba`, `hopfield*`, `gated_delta`, `default` — run_naming refs updated to `segmentation.*`); babi/pg19 configs keep working via the aliases. Supporting changes: the shared helpers (`detect_pair_ends`/`divide_pairs`/`pair_segment_spans`) are now module-level in `grad_memgpt.py` and imported by the fork (single source of division truth for models + collator); curriculum `MODEL_PARAM_MAP` accepts unified override names routed per-model; `generate_run_name` reads `segmentation:` (adaptive-fallback) and marks pair-aware runs `_pa{N}`; adaptive + segmentation/pair-aware also disables context truncation like hopfield runs.
