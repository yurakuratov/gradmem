...
class GradRMTConfig(PretrainedConfig):
    """
    Configuration class for GradRMT.
    """
...
class InputPrefixMemoryBackend(MemoryBackend):
    ...
    def prepare_batch(self, context_input_ids, query_input_ids, pad_id, chunk_size):
        o = self.owner
        ...
        ctx_emb, lm_labels, mask = [self.split_tensor(t, chunk_size) for t in [ctx_emb, lm_labels, mask]]
        n_chunks = len(ctx_emb)

        if o.attn_implementation in ('jvp_flash', 'hvp_semi_manual'):
            for i in range(n_chunks):
                pad_list = [0, -(ctx_emb[i].size(1) + mem_offset) % 32, 0, 0]
                mask[i] = F.pad(mask[i], pad_list, "constant", 0)
                lm_labels[i] = F.pad(lm_labels[i], pad_list, "constant", -100)
                ctx_emb[i] = F.pad(ctx_emb[i], [0, 0] + pad_list, "constant", 0)

        batch_ctx["x_ctx"] = ctx_emb
    ...
    def build_write_inputs(self, memory_state, batch_ctx):
        ...
        if o.n_ctrl_tokens > 0:
            x_ctx = torch.cat(
                [batch_ctx["write_st_batch"], mem_inp, batch_ctx["write_end_batch"], batch_ctx["x_ctx"]],
                dim=1,
            )
        else:
            x_ctx = torch.cat([mem_inp, batch_ctx["x_ctx"]], dim=1)
...
class LoraMemoryBackend(MemoryBackend):
    ...
    def prepare_batch(self, context_input_ids, query_input_ids, pad_id, chunk_size):
        ...
        if o.attn_implementation in ('jvp_flash', 'hvp_semi_manual'):
            for i in range(n_chunks):
                pad_n = (-x_ctx[i].size(1)) % 32
                if pad_n:
                    x_ctx[i] = F.pad(x_ctx[i], [0, 0, 0, pad_n], "constant", 0)
                    lm_labels[i] = F.pad(lm_labels[i], [0, pad_n], "constant", -100)
                    mask[i] = F.pad(mask[i], [0, pad_n], "constant", 0)
...
class GradRMT(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if _is_main_process():
            logger.info(f"GradRMT memory params (backend={self.memory_backend}): total={n_memory_params}")
    ...
    @staticmethod
    def expand_batch_ctx(batch_ctx):
        list_keys = [k for k, v in batch_ctx.items() if k in ['x_ctx', 'lm_labels', 'mask']]
        scalar = {k: v for k, v in batch_ctx.items() if k not in ['x_ctx', 'lm_labels', 'mask']}
