...
| forward integration (train-only gate, boundary loop, loss assembly) | `forward`: `star_active` at `:1179`, boundary call at `:1428`, `+λ·KL` at `:1558` |
| probe construction (shared with the forgetting eval) | `build_kv_probe_queries` (`run_gradmemgpt_on_kv_retrieval.py:459`) |
| train collator attaches probes when `star_weight>0` | `run_gradmemgpt_on_kv_retrieval.py:1313` (single-stage), `:1469` (curriculum stages) |
...
### The pilot ladder (STAR paper Table-3 replication)

```bash
CFG=configs/gradmemgpt/kv_retrieval/adaptive_star.yaml

# 1. baseline: plain-SGD multi-segment write
...
# 3. full STAR (worst-case), then the small grid
